...
const Home = () => {
    ...
    useEffect(
        ...
        }, [fetchTopAnime, fetchTopManga, filterValue, setError]
    )
    return (
        <View style={{backgroundColor: darkTheme ? "#282C5" : "white"}}>
            <View style={styles.container}>
                <View style={{ flexDirection: 'row', padding: 10 }}>
                    <Text style={{ fontSize: 20, color: darkTheme ? 'white' : 'black' }}>Filter</Text>
                    <MaterialCommunityIcons name='filter' size={20} color='white' style={{ padding: 5 }}/>
                </View>
                <Picker 
                    selectedValue={filterValue}
                    dropdownIconColor={darkTheme ? 'white' : 'black'}
                    style={{ height: 50, width: 150, color: darkTheme ? 'white' : 'black'}}
                            ...
                            <Carousel 
                                ...
                                renderItem={({ item }) => (
                                    <TouchableOpacity 
                                        ...
                                        <View style={{ height: windowHeight, width: windowWidth, transform: [{ scaleY: -1 }]}} key={item.mal_id}>
                                            <Image source={{ url: item.images.jpg.large_image_url }} style= {{ height: "100%", resizeMode: "cover", width: windowWidth }}/>
                                            <ImageBackground blurRadius={30} style={styles.footer} source={{ url: item.images.jpg.large_image_url}}>
                                                <Text style={{
                                                        fontSize: 22,
            ...
            {
                Target ? <ModalAnime Visible={Visible} setVisible={setVisible} Target={Target}/> : <View></View>
            }
        </View>
    );
}
